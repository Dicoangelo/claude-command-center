    ...
    with open(flow_file) as f:
        for line in f:
            try:
                ...
                # Source data uses "score" field, not "flow_score"
                score = entry.get("score") or entry.get("flow_score", 0)
                flow_scores.append(score)

                # Track peak hours — source uses "timestamp" (ISO string), not "ts" (epoch)
...
if cmd_file.exists():
    with open(cmd_file) as f:
        for line in f:
            try:
                entry = json.loads(line)
                cmd_stats["total"] += entry.get("count", 1)
