...
class PredictiveRouter:
    ...
    def _load_routing_history(self) -> List[Dict]:
        ...
        DATA_DIR / "session-outcomes.jsonl"
    ...
    def _compute_correlations(self) -> Dict:
        ...
        return {model: sum(scores) / len(scores) if scores else 3.0 for model, scores in model_quality.items()}
...
class ContextPredictor:
    ...
    def _analyze_tool_sequences(self) -> Dict:
        ...
        for tool, nexts in sequences.items():
            ...
            patterns[tool] = {
                next_tool: count / total for next_tool, count in sorted(nexts.items(), key=lambda x: -x[1])[:5]
            }
...
class SessionPredictor:
    ...
    def _get_recommendation(self, prob: float, hour: int, model: str) -> str:
        if prob < 0.5:
            return "Consider waiting for a better hour or using a different model"
        elif prob < 0.7:
...
class CCCIntelligence:
    ...
    def analyze_query(self, query: str) -> Dict:
        ...
        # Predict success
        success = self.session.predict_success(routing["recommended_model"], routing["detected_complexity"])
