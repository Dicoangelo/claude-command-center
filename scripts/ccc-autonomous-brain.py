        ...
        return [
            {
                ...
                "fix_effectiveness": {
                    k: v["success"] / v["total"] if v["total"] > 0 else 0 for k, v in fix_success.items()
                },
            }
        ]
        ...
        if logs_dir.exists():
            for log in logs_dir.glob("*.log"):
                try:
                    ...
                    age_days = (
                        datetime.now(LOCAL_TZ) - datetime.fromtimestamp(log.stat().st_mtime, tz=LOCAL_TZ)
                    ).days or 1
            ...
            for log in logs_dir.glob("*.log"):
                try:
                    ...
                    if size_mb > threshold_mb:
                        ...
                        log.write_text("\n".join(keep))
                        actions.append({"type": "prerotate", "file": log.name, "freed_mb": size_mb * 0.7})
                except Exception:
                    pass
            ...
            if now.hour == peak - 1 and now.minute >= 45:
                ...
                for d in critical:
                    if f"com.claude.{d}" not in result.stdout:
                        actions.append({"type": "warmup", "daemon": d, "reason": f"Peak hour {peak}:00 approaching"})
...
class FeedbackLoop:
    ...
    def run_experiment(self, exp_id: str) -> Dict:
        ...
        exp["results"] = {"baseline": baseline, "after": after, "improvement": improvement, "success": improvement >= 0}
    ...
    def _measure_health(self) -> Dict:
        ...
        try:
            result = subprocess.run(
                ["python3", str(SCRIPTS_DIR / "ccc-self-heal.py"), "--json"], capture_output=True, text=True, timeout=60
            )
