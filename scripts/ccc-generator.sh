...
# ═══════════════════════════════════════════════════════════════
# GATHER DATA (SQLite is single source of truth)
# ═══════════════════════════════════════════════════════════════

CCC_SQL="$HOME/.claude/scripts/ccc-sql-data.py"

echo "  📊 Loading stats from SQLite..."
STATS_DATA=$(python3 "$CCC_SQL" stats 2>/dev/null || echo "$DEFAULT_STATS")
...
def get_project_stats(path, name, tech, alias):
    ...
    file_types = {}
    try:
        for ext in ['ts', 'tsx', 'js', 'jsx', 'py', 'sh', 'json', 'md', 'css', 'html']:
            result = subprocess.run(
                ["find", str(path), "-type", "f", "-name", f"*.{ext}", "-not", "-path", "*/node_modules/*", "-not", "-path", "*/.git/*"],
    ...
    # Get line count (code files only)
    lines_raw = 0
    try:
        result = subprocess.run(
            ["find", str(path), "-type", "f", "(", "-name", "*.ts", "-o", "-name", "*.tsx", "-o", "-name", "*.js", "-o", "-name", "*.jsx", "-o", "-name", "*.py", ")", "-not", "-path", "*/node_modules/*", "-not", "-path", "*/.git/*", "-exec", "cat", "{}", "+"],
            capture_output=True, text=True, timeout=10
        )
        lines_raw = len(result.stdout.split('\n'))
    except:
        pass
    lines = f"{lines_raw/1000:.1f}K" if lines_raw > 1000 else str(lines_raw)

    # Last commit date and branch
    last_commit = ""
    branch = ""
    try:
        result = subprocess.run(
    ...
    try:
        result = subprocess.run(
            ["git", "-C", str(path), "rev-list", "--count", "--since=7 days ago", "HEAD"],
            capture_output=True, text=True, timeout=5
        )
        if result.returncode == 0:
...
echo "  🎯 Loading routing metrics from SQLite..."
ROUTING_TMP="/tmp/routing-data-$$.json"
python3 "$CCC_SQL" routing > "$ROUTING_TMP" 2>/dev/null || echo '{"totalQueries":0,"dataQuality":0.0,"feedbackCount":0,"targetQueries":5000,"targetDataQuality":0.80,"targetFeedback":1000}' > "$ROUTING_TMP"

# Legacy inline routing block preserved but bypassed
if false; then
python3 << 'ROUTINGEOF' > /dev/null
import json
...
ROUTINGEOF
fi  # end of legacy routing block
...
echo "  📋 Loading session outcomes from SQLite..."
SESSION_OUTCOMES_TMP="/tmp/session-outcomes-data-$$.json"
# Query SQLite directly instead of reading JSONL
python3 "$CCC_SQL" outcomes > "$SESSION_OUTCOMES_TMP" 2>/dev/null || echo '{"sessions":[],"totals":{},"daily":[]}' > "$SESSION_OUTCOMES_TMP"

# Legacy JSONL block removed — SQLite is the source of truth now
...
result = {
    ...
    "review": {"due_count": 0, "upcoming_count": 0, "items": [], "byCategory": {}},
    "recent_learnings": [],
    "error_patterns": [],
    "projects": {},
    "sources": {},
...
if db_path.exists():
    ...
    try:
        ...
        rows = conn.execute("SELECT source, COUNT(*) as cnt FROM memory_items GROUP BY source ORDER BY cnt DESC").fetchall()
        result["sources"] = {row["source"]: row["cnt"] for row in rows}

        # Get learning categories breakdown
        rows = conn.execute("SELECT category, COUNT(*) as cnt FROM learnings GROUP BY category ORDER BY cnt DESC").fetchall()
        result["learningCategories"] = {row["category"]: row["cnt"] for row in rows}

        # Daily memory growth (last 30 days)
        rows = conn.execute("SELECT date(created_at) as day, COUNT(*) as cnt FROM memory_items WHERE created_at >= date('now', '-30 days') GROUP BY day ORDER BY day").fetchall()
        result["dailyGrowth"] = [{"date": r["day"], "count": r["cnt"]} for r in rows]

        # Daily learnings (last 30 days)
        rows = conn.execute("SELECT date(date) as day, COUNT(*) as cnt FROM learnings WHERE date >= date('now', '-30 days') GROUP BY day ORDER BY day").fetchall()
        result["dailyLearnings"] = [{"date": r["day"], "count": r["cnt"]} for r in rows]
        ...
        rows = conn.execute("SELECT category, pattern, count, solution FROM error_patterns ORDER BY count DESC LIMIT 8").fetchall()
        ...
        result["review"]["upcoming_count"] = conn.execute("SELECT COUNT(*) FROM reviews WHERE next_review > ?", (today,)).fetchone()[0]
        rows = conn.execute("SELECT id, content, category, next_review FROM reviews WHERE next_review <= ? ORDER BY next_review LIMIT 10", (today,)).fetchall()
        ...
        rows = conn.execute("SELECT category, COUNT(*) as total, SUM(CASE WHEN next_review <= ? THEN 1 ELSE 0 END) as due FROM reviews GROUP BY category ORDER BY total DESC", (today,)).fetchall()
        result["review"]["byCategory"] = {r["category"]: {"total": r["total"], "due": r["due"]} for r in rows}
...
result = {
    ...
    "fate_distribution": {"success": 0, "abandon": 0, "partial": 0, "unknown": 0},
    "flow_history": [],
    "routing_history": [],
    "routing_sqlite": {"byModel": {}, "daily": [], "total": 0},
    "fate_daily": []
}
...
if fate_file.exists():
    try:
        ...
        # Fate distribution
        dist = {"success": 0, "abandon": 0, "partial": 0, "unknown": 0}
        for p in predictions:
            ...
            if pred in dist:
                ...
                dist["unknown"] += 1
        result["fate_distribution"] = dist

        # Daily fate accuracy (last 30 days)
        from collections import defaultdict
        daily_fate = defaultdict(lambda: {"total": 0, "correct": 0})
        for p in predictions:
            ts = p.get("timestamp", "")
            day = ts[:10] if len(ts) >= 10 else ""
            if day:
                daily_fate[day]["total"] += 1
                if p.get("correct", False):
                    daily_fate[day]["correct"] += 1
        result["fate_daily"] = [
            {"date": k, "total": v["total"], "correct": v["correct"], "accuracy": round(v["correct"]/v["total"]*100,1) if v["total"]>0 else 0}
            for k, v in sorted(daily_fate.items())[-30:]
        ]
    except: pass

# Load routing data from SQLite (more complete than JSONL)
import sqlite3 as _sq
claude_db = home / ".claude" / "data" / "claude.db"
if claude_db.exists():
    try:
        _conn = _sq.connect(str(claude_db))
        _conn.row_factory = _sq.Row
        # Model distribution
        rows = _conn.execute("SELECT selected_model, COUNT(*) as cnt FROM routing_decisions WHERE selected_model IS NOT NULL GROUP BY selected_model ORDER BY cnt DESC").fetchall()
        result["routing_sqlite"]["byModel"] = {r["selected_model"]: r["cnt"] for r in rows}
        result["routing_sqlite"]["total"] = sum(r["cnt"] for r in rows)
        # Daily DQ scores + routing
        rows = _conn.execute("SELECT substr(timestamp,1,10) as day, COUNT(*) as cnt, ROUND(AVG(dq_score),3) as avg_dq, ROUND(AVG(complexity),3) as avg_cx FROM routing_decisions GROUP BY day ORDER BY day DESC LIMIT 30").fetchall()
        result["routing_sqlite"]["daily"] = [{"date": r["day"], "count": r["cnt"], "avg_dq": r["avg_dq"], "avg_complexity": r["avg_cx"]} for r in reversed(list(rows))]
        _conn.close()
    except: pass
...
projects = [
    ('OS-App', Path.home() / 'projects/apps/OS-App'),
    ('CareerCoach', Path.home() / 'projects/apps/CareerCoachAntigravity'),
    ('ResearchGravity', Path.home() / 'projects/apps/researchgravity'),
]
...
echo "  🩹 Loading recovery data from SQLite..."
RECOVERY_TMP="/tmp/recovery-data-$$.json"
python3 "$CCC_SQL" recovery > "$RECOVERY_TMP" 2>/dev/null || echo '{"stats":{"total":0,"autoFix":0,"autoFixRate":0,"successRate":0},"categories":{},"outcomes":[],"timeline":[],"successByCategory":{},"matrix":[]}' > "$RECOVERY_TMP"

# Legacy inline recovery block preserved but bypassed
if false; then
python3 << 'RECOVERYEOF' > /dev/null
import json
from pathlib import Path
from datetime import datetime, timedelta
from collections import Counter, defaultdict
...
print(json.dumps(result))
RECOVERYEOF
fi  # end of legacy recovery block
...
# Daily stats (all-time from first significant event)
now = datetime.now()
# Skip rogue events before 2026 — real sessions started Jan 5, 2026
jan2026 = int(datetime(2026, 1, 1).timestamp())
cursor = conn.execute("SELECT MIN(timestamp) as earliest FROM tool_events WHERE timestamp > ?", (jan2026,))
earliest_row = cursor.fetchone()
earliest_ts = earliest_row["earliest"] if earliest_row and earliest_row["earliest"] else int((now - timedelta(days=14)).timestamp())
start_date = datetime.fromtimestamp(earliest_ts).replace(hour=0, minute=0, second=0)
num_days = (now - start_date).days + 1

cursor = conn.execute("SELECT date(timestamp, 'unixepoch') as day, COUNT(*) as total, SUM(CASE WHEN tool_name = 'Write' THEN 1 ELSE 0 END) as writes, SUM(CASE WHEN tool_name = 'Edit' THEN 1 ELSE 0 END) as edits, SUM(CASE WHEN tool_name = 'Bash' THEN 1 ELSE 0 END) as bash, SUM(CASE WHEN tool_name = 'Read' THEN 1 ELSE 0 END) as reads FROM tool_events WHERE timestamp > ? GROUP BY day ORDER BY day", (jan2026,))
...
result["daily"] = [
    daily_data.get((start_date + timedelta(days=i)).strftime("%Y-%m-%d"), {
...
# Daily usage (all-time from first significant event)
tool_jan2026 = int(datetime(2026, 1, 1).timestamp())
cursor = conn.execute("SELECT MIN(timestamp) as earliest FROM tool_events WHERE timestamp > ?", (tool_jan2026,))
earliest_row = cursor.fetchone()
earliest_ts = earliest_row["earliest"] if earliest_row and earliest_row["earliest"] else int((now - timedelta(days=7)).timestamp())
tool_start_date = datetime.fromtimestamp(earliest_ts).replace(hour=0, minute=0, second=0)
tool_num_days = (now - tool_start_date).days + 1

cursor = conn.execute("SELECT date(timestamp, 'unixepoch') as day, COUNT(*) as count FROM tool_events WHERE timestamp > ? GROUP BY day ORDER BY day", (tool_jan2026,))
daily_data = {row["day"]: row["count"] for row in cursor.fetchall()}

result["daily"] = [
    {"date": (tool_start_date + timedelta(days=i)).strftime("%Y-%m-%d"),
     "count": daily_data.get((tool_start_date + timedelta(days=i)).strftime("%Y-%m-%d"), 0)}
    for i in range(tool_num_days)
]
...
result = {
    ...
    "selfHealDaily": [],
    "daemonHealCounts": {},
    "recoveryStats": {"total": 0, "successful": 0, "byType": {}, "byMethod": {}},
    "selfHealSummary": {"totalRuns": 0, "totalFixed": 0, "totalChecks": 0, "totalWarn": 0, "totalError": 0}
}
...
# --- Watchdog daily trends (all days from log) ---
from collections import defaultdict
watchdog_daily = defaultdict(lambda: {"down": 0, "loaded": 0, "healed": 0})
if watchdog_log.exists():
    try:
        for line in watchdog_log.read_text().strip().split('\n'):
            m = re.match(r'\[(\d{4}-\d{2}-\d{2})', line)
            if m:
                day = m.group(1)
                if 'DOWN:' in line: watchdog_daily[day]['down'] += 1
                if 'LOADED' in line: watchdog_daily[day]['loaded'] += 1
                if 'HEALED' in line: watchdog_daily[day]['healed'] += 1
    except:
        pass
result["watchdogDaily"] = [{"date": d, **watchdog_daily[d]} for d in sorted(watchdog_daily.keys())]

# --- Daemon heal counts (from full watchdog log) ---
daemon_heal_counts = defaultdict(int)
if watchdog_log.exists():
    try:
        for line in watchdog_log.read_text().strip().split('\n'):
            if 'LOADED' in line:
                daemon = line.split('LOADED')[-1].strip().replace('com.claude.', '')
                if daemon:
                    daemon_heal_counts[daemon] += 1
    except:
        pass
result["daemonHealCounts"] = dict(daemon_heal_counts)

# --- Self-heal daily health (from outcomes JSONL) ---
self_heal_outcomes_path = home / ".claude/data/self-heal-outcomes.jsonl"
sh_daily = defaultdict(lambda: {"ok": 0, "warn": 0, "error": 0, "fixed": 0, "runs": 0})
if self_heal_outcomes_path.exists():
    try:
        total_runs = 0
        total_fixed = 0
        total_checks = 0
        total_warn = 0
        total_error = 0
        for line in self_heal_outcomes_path.read_text().strip().split('\n'):
            if line.strip():
                d = json.loads(line)
                day = datetime.fromtimestamp(d.get('ts', 0), tz=LOCAL_TZ).strftime('%Y-%m-%d')
                sh_daily[day]['ok'] += d.get('ok', 0)
                sh_daily[day]['warn'] += d.get('warn', 0)
                sh_daily[day]['error'] += d.get('error', 0)
                sh_daily[day]['fixed'] += d.get('fixed', 0)
                sh_daily[day]['runs'] += 1
                total_runs += 1
                total_fixed += d.get('fixed', 0)
                total_checks += d.get('ok', 0) + d.get('warn', 0) + d.get('error', 0)
                total_warn += d.get('warn', 0)
                total_error += d.get('error', 0)
        result["selfHealSummary"] = {
            "totalRuns": total_runs,
            "totalFixed": total_fixed,
            "totalChecks": total_checks,
            "totalWarn": total_warn,
            "totalError": total_error
        }
    except:
        pass
result["selfHealDaily"] = [{"date": d, **sh_daily[d]} for d in sorted(sh_daily.keys())]

# --- Recovery stats from SQLite ---
...
if db_path.exists():
    try:
        ...
        c = conn.cursor()
        c.execute("SELECT COUNT(*), SUM(success) FROM recovery_events")
        row = c.fetchone()
        result["recoveryStats"]["total"] = row[0] or 0
        result["recoveryStats"]["successful"] = row[1] or 0
        c.execute("SELECT error_type, COUNT(*) FROM recovery_events GROUP BY error_type ORDER BY COUNT(*) DESC")
        result["recoveryStats"]["byType"] = {r[0]: r[1] for r in c.fetchall()}
        c.execute("SELECT recovery_method, COUNT(*) FROM recovery_events GROUP BY recovery_method ORDER BY COUNT(*) DESC")
        result["recoveryStats"]["byMethod"] = {r[0] or 'unknown': r[1] for r in c.fetchall()}
        conn.close()
    except:
        pass
...
def check_freshness(path, name, max_hours):
    if not path.exists():
        return {"name": name, "fresh": False, "age": "Missing", "hours": -1}
    ...
    return {"name": name, "fresh": fresh, "age": age_str, "hours": round(age_hours, 2), "maxHours": max_hours}
...
result["dataFreshness"] = [
    ...
    check_freshness(home / ".claude/data/claude.db", "Claude DB", 1),
    ...
    check_freshness(home / ".claude/logs/watchdog.log", "Watchdog Log", 2),
]
...
coevo_data = {
    ...
    "runtimePatterns": coevo_runtime.get('patterns', {}),
    "configWeights": coevo_config.get('weights', {}),
    "configThresholds": coevo_config.get('thresholds', {})
}

# Enrich pattern_trends with daily data (last 21 days) for charts
pt_file = Path.home() / '.claude/kernel/pattern-trends.json'
if pt_file.exists():
    try:
        with open(pt_file) as f:
            pt = json.load(f)
        daily_raw = pt.get('daily', {})
        # Get last 21 days sorted
        sorted_days = sorted(daily_raw.keys())[-21:]
        daily_list = []
        for day in sorted_days:
            entry = {"date": day}
            entry.update(daily_raw[day])
            daily_list.append(entry)
        pattern_trends['dailyList'] = daily_list
    except:
...
with open('$SESSION_OUTCOMES_TMP', 'r') as f:
    session_outcomes = safe_parse(f.read(), {"sessions":[],"totals":{},"qualityDist":{},"modelOutcomes":{},"sizeDist":{},"daily":[]})
