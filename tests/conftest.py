...
def tmp_db(tmp_path):
    ...
    if schema_path.exists():
        ...
        conn.executescript("""
            CREATE TABLE IF NOT EXISTS daily_stats (
                ...
                opus_tokens_in INTEGER DEFAULT 0, opus_tokens_out INTEGER DEFAULT 0, opus_cache_read INTEGER DEFAULT 0,
                sonnet_tokens_in INTEGER DEFAULT 0, sonnet_tokens_out INTEGER DEFAULT 0, sonnet_cache_read INTEGER DEFAULT 0,
                haiku_tokens_in INTEGER DEFAULT 0, haiku_tokens_out INTEGER DEFAULT 0, haiku_cache_read INTEGER DEFAULT 0,
    ...
    conn.executemany(
        """INSERT OR IGNORE INTO daily_stats
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        [
            ("2026-02-10", 80, 20, 5, 500000, 150000, 4000000, 100000, 30000, 800000, 10000, 3000, 50000, 12, 450, 120.50),
            ("2026-02-11", 95, 15, 3, 600000, 180000, 5000000, 80000, 25000, 700000, 5000, 2000, 30000, 15, 520, 145.30),
            ("2026-02-12", 110, 25, 8, 700000, 200000, 6000000, 120000, 35000, 900000, 15000, 5000, 60000, 18, 680, 175.80),
        ],
    )
    ...
    conn.executemany(
        """INSERT OR IGNORE INTO sessions
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        [
            ("sess-001", "/Users/test/project-a", "2026-02-12 10:00:00", "2026-02-12 11:30:00",
             "opus", 85, 120, 500000, 150000, 4000000, "completed", 0.85, 55.0),
            ("sess-002", "/Users/test/project-b", "2026-02-12 14:00:00", "2026-02-12 15:00:00",
             "sonnet", 40, 60, 200000, 60000, 1500000, "completed", 0.72, 18.0),
            ("sess-003", "/Users/test/project-a", "2026-02-12 16:00:00", None,
             "opus", 20, 30, 100000, 30000, 800000, None, None, 10.0),
        ],
    )

    conn.executemany(
        """INSERT OR IGNORE INTO tool_usage (tool_name, total_calls, success_count, failure_count, last_used, avg_duration_ms)
           VALUES (?, ?, ?, ?, ?, ?)""",
        [
...
def tmp_claude_dir(tmp_path, tmp_db):
    ...
    (claude_dir / "stats-cache.json").write_text(json.dumps({
        "sessions": 100, "messages": 5000, "tools": 2000, "tokens": 50000000
    }))
    ...
    (dash_dir / "claude-command-center.html").write_text(
        "<html><body><h1>CCC Dashboard</h1></body></html>"
    )
    ...
    # Data JSONL files
    (data_dir / "session-outcomes.jsonl").write_text(
        '{"session_id":"s1","outcome":"completed","quality":4}\n'
    )
    (data_dir / "tool-usage.jsonl").write_text(
        '{"tool":"Read","count":100}\n'
    )
    (data_dir / "tool-success.jsonl").write_text(
        '{"tool":"Read","success":98,"failure":2}\n'
    )
    (data_dir / "git-activity.jsonl").write_text(
        '{"type":"commit","repo":"test","message":"fix bug"}\n'
    )
    (data_dir / "routing-metrics.jsonl").write_text(
        '{"model":"opus","accuracy":0.85}\n'
    )
    (data_dir / "routing-feedback.jsonl").write_text(
        '{"model":"opus","correct":true}\n'
    )
